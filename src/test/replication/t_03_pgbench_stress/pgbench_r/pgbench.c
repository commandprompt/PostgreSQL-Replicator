/*
 * $Header: /var/lib/cvs/Mammoth/Replicator/testcase_perl/test3/pgbench_r/Attic/pgbench.c,v 1.1.2.1 2004/06/25 13:58:21 ggray Exp $
 *
...
usage()
{
	...
	fprintf(stderr, "(initialize mode): pgbench -i -I [-h hostname][-p port][-s scaling_factor][-U login][-P password][-d][dbname]\n");
}
...
//			memset(updata, '1', 14999);
//			memset(updata, (int) ((double)10*rand()/(10+1.0))+65, 1000);
//			updata[14999] = (char)0;
//			snprintf(sql, 20000, "update test1 set filler='%s' where id=%d", updata, 9);//(int) ((double)20*rand()/(100+1.0)));
//			printf("updated test1\n");
			break;
		case 2:
	...
	//		memset(updata, '1', 14999);
	//		memset(updata, (int) ((double)10*rand()/(RAND_MAX+1.0))+65, 1000);
	//		updata[14999] = (char)0;
	//		snprintf(sql, 20000, "update test1 set filler='%s' where id=%d", updata, (int) ((double)100*rand()/(RAND_MAX+1.0)));
	//		printf("updated test1\n");
	//			printf("%s\n", sql);
				break;
			}
...
//		"drop table test1",
//		"create table test1(id int, primary key(id), filler varchar)",
		"drop table tellers",
		...
		"drop table history",
	"create table history(pk serial,primary key(pk),tid int,bid int,aid int,delta int,mtime timestamp,filler char(22))"};
...
/*
	printf("init test1...");
	memset(updata, '1', 14998);
	updata[14999] = (char)0;
	for (i = 0; i < 100; i++)
	{
		snprintf(sql2, 20000, "insert into test1(id, filler) values(%d, '%s')", i, updata);
		res = PQexec(con, sql2);
		if (PQresultStatus(res) != PGRES_COMMAND_OK)
		{
			fprintf(stderr, "%s", PQerrorMessage(con));
...
main(int argc, char **argv)
{
	...
	int			is_initdata_mode = 0;		/* initialize mode? */
	...
	while ((c = getopt(argc, argv, "iIh:nvp:dc:t:s:U:P:CNSl")) != -1)
	{
		switch (c)
		{
			case 'i':
